...
func TestAccDataSourceResourceTypeTags_basic(t *testing.T) {
	dataSource := "data.huaweicloud_rms_resource_type_tags.test"
	dc := acceptance.InitDataSourceCheck(dataSource)
	name := acceptance.RandomAccResourceName()
	baseConfig := testDataSourceResourceTypeTags_base(name)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testDataSourceResourceTypeTags_basic(baseConfig),
...
func testDataSourceResourceTypeTags_basic(baseConfig string) string {
	return fmt.Sprintf(`
%[1]s

data "huaweicloud_rms_resource_type_tags" "test" {
...
func testDataSourceResourceTypeTags_base(name string) string {
	return fmt.Sprintf(`
